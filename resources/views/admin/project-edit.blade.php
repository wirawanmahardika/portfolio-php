<x-admin-layout>
  <div class="p-8 w-full flex flex-col gap-y-5 items-center">
    ...
    <form method="post" enctype="multipart/form-data" action="/admin/project/edit" class="w-1/2 mx-auto flex flex-col p-5 gap-y-5 ">
      @csrf
      ...
      <div class="flex flex-col">
        ...
        <input
          ...
          value="{{old('nama')}}"
        />
        @if ($errors->has("nama")) <span class="mt-1 text-red-500 italic text-sm">{{$errors->first('nama')}}</span> @endif
      </div>
      <div class="flex flex-col">
        ...
        <input
          ...
          value="{{old('github')}}"
        />
        @if ($errors->has("github")) <span class="mt-1 text-red-500 italic text-sm">{{$errors->first('github')}}</span> @endif
      </div>
      <div class="flex flex-col">
        ...
        <input
          ...
          value="{{old('web')}}"
        />
        @if ($errors->has("web")) <span class="mt-1 text-red-500 italic text-sm">{{$errors->first('web')}}</span> @endif
      </div>
      ...
      <div class="flex flex-col">
        ...
        <textarea name="deskripsi"
          value="{{old('deskripsi')}}"
          class="rounded outline-none border-2 border-black"
          rows="3"
        ></textarea>

        @if ($errors->has("deskripsi")) <span class="mt-1 text-red-500 italic text-sm">{{$errors->first('deskripsi')}}</span> @endif
      </div>
